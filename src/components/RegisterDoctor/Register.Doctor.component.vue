      ...
      <div class="form-body">
        ...
        <div class="form-row">
          <div class="form-group full-width">
            ...
            <input type="text" v-model="form.hospital_clinic" />
            <span v-if="errors.hospital_clinic" class="error">{{ errors.hospital_clinic }}</span>
          </div>
        </div>
        <div class="form-row">
          <div class="form-group full-width">
...
export default {
  ...
  setup() {
    ...
    const errors = ref({}); // Objeto para guardar mensajes de error

    const validateForm = () => {
      errors.value = {}; // Reiniciar los errores
      for (const key in form.value) {
        if (!form.value[key]) {
          errors.value[key] = `${key.charAt(0).toUpperCase() + key.slice(1).replace(/_/g, ' ')} is required.`;
        }
      }
      return Object.keys(errors.value).length === 0; // true si no hay errores
    };
